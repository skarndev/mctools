...
ModelParticleEmitterDef *ModelEditor::getParticles(){
	return part_m;

}
ParticleHelper *ModelEditor::getParticleData(){
	return NULL;
}
...
BoneHelper* ModelEditor::getBoneData(){
	...
	for(int i=0;i<header_m->nBones;i++){
		...
		for(int j=0;j<header_m->nAnimations;j++){
			if(bone_m[i].translation.nTimes!=0&&tr_timesub[j].n!=0){
			...
			if(bone_m[i].rotation.nTimes!=0&&ro_timesub[j].n!=0){
				QuatHelper* vHelp=new QuatHelper[ro_timesub[j].n];
				short *timestamp;SQuaternion *value;
				timestamp=getAnimblockData(timestamp,ro_timesub[j].ofs);
				value=getAnimblockData(value,ro_keysub[j].ofs);
				for(int k=0;k<ro_timesub[j].n;k++){
					vHelp[k].timestamp=timestamp[k];
					vHelp[k].value=value[k];
				}
				help[i].info[j].setRotation(vHelp,ro_timesub[j].n);
			}
			if(bone_m[i].scaling.nTimes!=0&&sc_timesub[j].n!=0){
				Vec3Helper* vHelp=new Vec3Helper[sc_timesub[j].n];
				short *timestamp;Vec3D *value;
