...
bool g_bForceCharacterModeBLPs = false;
...
HSB RGBToHSB(int r, int g, int b)
{
	...
	if (BMin > 0)
	{
		...
		grey = ((float)BMin / bright) + 0.5;
		sat = (100 - (((float)grey/255) * 100)) + 0.5;
		bright = int((bright * 100) + 0.5);
...
void SortPalette(UCHAR* palette, int numEntries)
{
	THSBList hsbList;
...
bool ConvertPNGToBLP(const char* pszInputFileName, const char* pszOutputFileName)
{
		...
		g_bSavePaletteWithAlphaToRGBA)
	{
		...
		for (ii = 0; ii < pixels; ++ii)
		{
			...
			a8p8Buffer[ii*2 + 1] = UCHAR(float(pImgBuffer[ii*4 + 3]));
		}
	...
	if (g_bForceCharacterModeBLPs)
	{
		if (FMT_X8R8G8B8 == aPng.format || 
			FMT_A8R8G8B8 == aPng.format)
		{
			// Palettize.
			...
			// Delete the old png buffer
			delete[] aPng.buffer;
			// Set it to our palettized buffer.
			aPng.buffer = palPlane;

			// Copy the palette into the png struct.
			for (int iColor = 0; iColor < 256; ++iColor)
			{
				aPng.palette[iColor].red = palette[iColor*3 + 0];
				aPng.palette[iColor].green = palette[iColor*3 + 1];
		...
		else if (aPng.format == FMT_A8P8)
		{
			...
			for (int ii = 0; ii < pixels; ++ii)
				palPlane[ii] = pImgBuffer[ii*2];

			// Replace the buffer with the p8 one.
			delete[] aPng.buffer;
			aPng.buffer = palPlane;
		...
		if (aPng.format != FMT_P8)
		{
			printf("ERROR: -h mode internal error, unexpected PNG format.\n");
			return false;
		}

		if (1.0f != g_fGammaFactor)
		{
			printf("WARNING: -g option overridden by -h mode.\n");
			g_fGammaFactor = 1.0f;
		}
	}
	...
	if (1.0f != g_fGammaFactor)
	{
		...
		else if (FMT_P8 == aPng.format)
		{
			UCHAR* newBuffer = new UCHAR[aPng.width * aPng.height * 2];

			for (int ii = 0; ii < aPng.width * aPng.height; ++ii)
			{
				newBuffer[ii*2 + 0] = aPng.buffer[ii];
				newBuffer[ii*2 + 1] = 255;
			}

			delete[] aPng.buffer;
			aPng.buffer = newBuffer;
			aPng.format = FMT_A8P8;
		}
	}
...
void Usage()
{
	...
	printf("-h : Force cHaracter texture format (palettized, no alpha) when making BLPs.\n");
}
...
int main(int argc, char* argv[])
{
	...
	for (iArg = 1; iArg < argc; ++iArg)
	{
		if ('-' == argv[iArg][0])
		{
			...
			case 'h':
			case 'H':
				g_bForceCharacterModeBLPs = true;
				break;
	...
	if (g_bForceCharacterModeBLPs && g_bSavePaletteWithAlphaToRGBA)
	{
		printf("ERROR: -r and -h cannot both be specified.\n");
		result = -1;
		goto Finish;
	}
